    ...
    <div class="pc-container">
        <div class="pc-content">
            ...
            <div class="max-w-3xl bg-white p-6 rounded shadow">
                ...
                <h2 class="text-lg font-semibold mb-4">Edit Sub Category</h2>

                <form action="{{ url('/modify-expenses-subcategory/'.$subcategories->id) }}" method="POST">
                    @csrf

                    <!-- Parent Category -->
                    <select name="category_id"
                        class="w-full border rounded px-4 py-2 mb-3" required>
                        <option value="">Select Category</option>
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}"
                                {{ old('cat_id', $subcategories->cat_id) == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
