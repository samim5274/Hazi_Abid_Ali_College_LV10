...
<body class="bg-gray-50 font-sans">
            ...
            <div class="max-w-full overflow-x-auto p-6 bg-white shadow-lg rounded-lg">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button id="openWithModal" 
                        ...
                        <i class="fa-solid fa-plus-circle"></i> Add Withdraw
                    </button>
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($withdrawDetails as $withdraw)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $loop->iteration }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($withdraw->date)->format('d M, Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $withdraw->bank->bank_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">৳ {{ $withdraw->amount }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <a href="{{ url('/edit-bank-withdraw/'.$withdraw->id) }}"
                                    class="text-indigo-600 hover:text-indigo-800 font-medium flex items-center gap-1">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>

                                <!-- Delete Button -->
                                <form action="{{ url('/delete-bank-withdraw/'.$withdraw->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                    @csrf
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-gray-500">No withdraws found for today.</td>
                        </tr>
                        @endforelse
                        <tfoot class="bg-gray-100 font-semibold">
                            <tr>
                                <td colspan="3" class="px-6 py-4 text-right">Total:</td>
                                <td class="px-6 py-4 whitespace-nowrap">৳ {{ $withdrawDetails->sum('amount') }}/-</td>
                                <td colspan="3"></td>
                            </tr>
                        </tfoot>
                    </tbody>
    ...
    <script>
        ...
        openWithModal.addEventListener('click', () => {
